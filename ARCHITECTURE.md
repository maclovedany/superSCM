...
#### `lib/agent/` — AI Agent 계층 (STEP 16)

6회차에서 더한 계층이다. **LLM 은 숫자를 만들지 않는다.** Tool 이 화면과 같은 `lib/scm.ts`
함수를 부르고, 모델은 그 결과를 고르고 설명한다.

| 파일 | 한 줄 책임 | 절대 하지 않는 일 |
|---|---|---|
| `lib/agent/schema.ts` | 답변의 모양(JSON Schema)과 파싱 · 산출 불가 생성 | 계산 · 통신 |
| `lib/agent/tools.ts` | Tool 4종과 Registry · 역할 필터 | Supabase 직접 조회 · 새 SCM 계산 |
| `lib/agent/llm.ts` | 모델과의 HTTP 통신만 | SCM 계산 · 예외 던지기 |
| `lib/agent/guardrail.ts` | 답변 속 숫자를 툴 반환값과 대조 | 없는 값을 대신 생성 |
| `lib/agent/orchestrator.ts` | 순서와 제한(6회 · 60초)을 통제 | 데이터 직접 계산 |
| `lib/agent/conversation.ts` | 대화 저장 (**이 계층에서 유일하게 DB 를 부른다**) | 답변을 없애는 예외 던지기 |

Tool 4종은 지금 데이터로 답할 수 있는 것만이다 — `getDemandProfile` · `getForecastAccuracy` ·
`getStockoutRisk` · `getLeadtimeStats`. 재고 전개 · 안전재고 · 발주 추천 · 알림 Tool 은 그
데이터 기능을 먼저 만든 뒤에 더한다. 이름만 먼저 만들면 실행 오류이거나 환각이다.

권한은 두 번 검사한다. 모델에게는 그 역할이 부를 수 있는 Tool 만 보여 주고(1차), 실행
직전에 역할을 다시 확인한다(2차). 화면에서 안 보이는 것은 보안이 아니다.
