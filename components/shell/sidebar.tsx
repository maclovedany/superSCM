...
import { menuGroupsFor, type AppRole, type MenuItem } from '@/lib/menu';
...
// ★ Task 15 fix round 1(C2-1) — Topbar와 같은 이유로 실습 취합 주기 표시를 함께 내려받는다.
export default function Sidebar({ role, permissionCodes, baseMonth, practiceCycle = false }: { role: AppRole; permissionCodes: readonly string[]; baseMonth: BaseMonthDisplay; practiceCycle?: boolean }) {
  const groups = menuGroupsFor(role, new PermissionSet(permissionCodes));
  const formatted = formatBaseMonthKorean(baseMonth.planMonth);
  return <aside className="sidebar"><Link href="/dashboard" className="brand"><span className="brand-mark">SCM</span><span className="brand-copy"><strong>SCM Intelligence</strong><span>월간 발주계획</span></span></Link><div className="shell-nav">{groups.map((group) => <MenuGroup key={group.label} label={group.label} items={group.items} />)}</div><div className="sidebar-foot"><b><BaseMonthValue formatted={formatted} reasonCode={baseMonth.reasonCode} suffix=" 발주계획" /></b>{practiceCycle ? <> <span className="tag amber" title="실습용으로 연 취합 주기입니다.">실습</span></> : null}<br />Supabase analytics · Phase 2</div></aside>;
}
