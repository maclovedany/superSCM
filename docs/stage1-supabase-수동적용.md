...
## 16. 큐 — 다음 기회에 묻어갈 것 (새 마이그레이션을 만들지 않는다)

§0-3 은 "이미 적용한 파일은 절대 다시 고치지 않는다"입니다. 아래 항목은 **그 규칙을 지키기
위해 지금 고치지 않고 대기**시키는 것들입니다. 주석 하나를 위해 새 번호의 보정
마이그레이션을 만드는 것은 과하므로, **다른 실질적인 이유로 그 객체를 재정의할 때 함께
갱신**합니다.

### 16-1. `analytics.v_shipment_monthly_item` 의 뷰 주석에 날짜 없는 관측치가 있다

`20260912001000_chart_demand_shipment_views.sql:369` 의 `comment on view` 안에 이 문장이
있고, **DB 가 그 텍스트를 그대로 들고 있습니다**(`\d+` 또는 `obj_description` 으로 보입니다):

```
실측: v_shipment_trend가 10,228행 중 1000행만 반환
```

**주장 자체는 지금도 참입니다** — PostgREST 1,000행 상한은 실재하고, 이 뷰를 필터 없이
조회하면 안 된다는 지시도 유효합니다. 낡은 것은 **모집단 수치 하나**입니다. `10,228` 은
HOC 팬아웃 수정(§15) **이전** 값이고 2026-09-13 재측정 기준 **10,198** 입니다.

문제는 숫자가 아니라 **날짜가 함께 있지 않다는 것**입니다. 같은 관측이 같은 파일 `:17` 에도
있지만 거기에는 바로 위에 `실측(배포 DB 스냅샷, 2026-09-13)` 이라는 머리가 붙어 있어
"그때의 관측"임이 드러납니다. 반면 `:369` 의 텍스트는 **DB 안에서 그 머리와 분리되어**
살기 때문에, `\d+ analytics.v_shipment_monthly_item` 을 뜨는 사람은 **날짜 없는 "실측
10,228"** 을 현재 사실로 읽게 됩니다.

**할 일**: 이 뷰를 다른 이유로 `create or replace view` 할 일이 생기면, 그때 함께 발행되는
`comment on view` 에 **관측 날짜(2026-09-13 07:43 · `4bc982e`)와 현재 모집단(10,198)** 을
넣어 주세요. `create or replace view` 는 어차피 `comment on view` 를 다시 발행하므로
**추가 비용이 0** 입니다.

**살아 있는 참조는 따로 갱신했습니다** — `lib/analytics/repository.ts` 의 같은 관측에는
날짜·조건·현재값이 들어가 있습니다. 지금 값을 찾는 사람은 그쪽을 봅니다.
