...
Ruling(내가 나를 잘못 비난한 기록의 정정): 적용 직전 가드가 거짓 주석을 탐지해 적용을 중단시킨 것을 나는 "내 패턴이 구별을 못 해 낸 거짓 경보"로 적었다. **틀렸다 — 가드는 옳게 발동했다.** 커밋 시점별로 측정하면 `71dccc5` 에 거짓 주장의 핵심어 `되살아난다` 가 **1회** 남아 있었다(첫 정정이 거짓 문장을 **인용 형태로** 그대로 남겼기 때문이다). 즉 그 순간 거짓 서술은 실재했고 적용 중단이 정답이었다. 그 뒤 구현자가 `6ce460b` 로 인용까지 제거해(내 중단과 재시도 사이에 들어왔다) 재시도 때 0회가 된 것이다. 패턴 선택이 부정확했던 것은 사실이다(`'0400을 단독'` 은 정정된 부정문에도 걸리며 `6ce460b` 이후에도 1회 남아 있다) — 그러나 **부정확한 패턴이 우연히 맞은 것이 아니라, 탐지 대상이 실제로 있었다.** 교훈은 "내 가드를 의심하라"가 아니라 **"가드가 발동하면 그 시점의 상태를 커밋별로 확인해 원인을 귀속하라"** 다. 스스로의 실수 목록도 측정 없이 늘리면 기록이 거짓이 된다.
