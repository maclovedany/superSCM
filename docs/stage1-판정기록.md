...
Ruling: 검증은 **성공과 실패가 다른 관측을 내는 형태**로 만든다. 이 작업에서 두 번 어겼다 — (1) 배포에서 temp 테이블로 롤백을 시험해, temp 는 세션 종료로도 사라지므로 "롤백"과 "세션 정리"를 구분할 수 없었다(실제 테이블로 다시 해서 `to_regclass = f` 로 증명), (2) 거짓 주석 제거를 확인하려 `'0400을 단독'` 을 grep 했는데 그 구절이 **정정된 부정문 안에 살아남아** 거짓 경보를 냈다(올바른 판별자는 거짓 주장의 핵심어 `되살아난다` = 0회, 그리고 정정 표식의 존재). 가드의 패턴은 내가 신경 쓰는 것을 **구별**해야 하고, 빈 grep 결과를 "없음"의 단독 증거로 쓰지 않는다.
