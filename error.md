...
**지시 전 상태를 판정할 때는 그 시점의 HEAD를 기준으로 확인합니다 — 지시의 결과물인
나중 커밋을 보면 인과가 뒤집힙니다.** 이 프로젝트에서 같은 형태의 실수가 두 번
났습니다. 첫 번째: `f49e619`/`bbe160c` 논쟁에서, 라운드 2 리뷰가 지적한 시점의
실제 HEAD(`f49e619`)를 확인하지 않고 그보다 나중 커밋(`bbe160c`, 그 지적에 대한
응답)을 근거로 "리뷰가 오해였다"고 잘못 결론지었습니다(바로 위 항목). 두 번째:
팀장이 "두 항목이 빠졌다"고 지적한 시점의 실제 HEAD는 `1966d9a`였고(그 커밋엔
정말로 두 항목이 없었습니다), 그런데 그 지적에 답할 때는 이미 같은 addendum의
앞선 요청에 응답해 만들어 둔 `9eb6f77`(두 항목을 이미 포함)를 grep해 "지적이
틀렸다"고 잘못 결론지었습니다 — 지시를 받은 뒤 내가 만든 결과물로 지시 전 상태를
판정한 것입니다. 두 경우 모두 "근거 없이 지적을 따르지 않고 검증한다"는 습관
자체는 옳았지만, **비교 대상의 시점**(그 지적이 나온 순간의 HEAD가 무엇이었는지)을
확인하지 않아 결론이 뒤집혔습니다. 다음에는 상대가 언급한 커밋 해시나 "그때"를
명시적으로 짚어 그 시점의 상태를 먼저 확인한 뒤 판단합니다.
