...
export type AvailableStockRow = {
   ...
   *  analytics.v_open_po_data_status + OpenPoStatusBanner가 화면 수준에서 한 번 안내합니다) */
...
/**
 * analytics.v_open_po_data_status 한 행 — Open PO(참고 열) 계산이 지금 출처 미확인 또는
 * 파싱 불가 데이터에 걸려 있는지 화면 수준에서 한 번 안내하기 위한 상태.
 *
 ...
 */
export type OpenPoDataStatus = {
  hasUnverifiedSource: boolean;
  hasUnparseable: boolean;
};

export function normalizeOpenPoDataStatus(row: Record<string, unknown> | null): OpenPoDataStatus | null {
  if (!row) return null;
  return {
    hasUnverifiedSource: Boolean(value(row, ['has_unverified_source'])),
    hasUnparseable: Boolean(value(row, ['has_unparseable'])),
  };
}

export const OPEN_PO_STATUS_BANNER_TITLE = 'Open PO 참고 열 안내';

export function openPoStatusBannerMessage(status: OpenPoDataStatus): string {
  if (status.hasUnverifiedSource && status.hasUnparseable) {
    return '출처가 확인되지 않은 발주·입고 데이터와 숫자로 읽을 수 없는 값이 함께 있어, 해당 품목의 Open PO 참고 열은 비어 있습니다. 정식 업로드로 발주 데이터가 들어오면 채워집니다.';
  }
  if (status.hasUnverifiedSource) {
    return '출처가 확인되지 않은(정식 업로드 경로를 거치지 않은) 발주·입고 데이터가 있어, 해당 품목의 Open PO 참고 열은 비어 있습니다 — 지어낸 숫자를 보여주지 않기 위해서입니다. 정식 업로드로 발주 데이터가 들어오면 채워집니다.';
  }
  return '일부 발주·입고 데이터를 숫자로 읽을 수 없어, 해당 품목의 Open PO 참고 열은 비어 있습니다. 원본 데이터를 확인해 주세요.';
}
