-- ⚠ 폐기됨 (2026-08-27 · STEP 2)
--
-- 이 파일은 anon 에게 for all using(true) 를 줍니다.
-- publishable 키는 브라우저에 노출되므로 키를 가진 누구나 정책값을 고칠 수 있습니다.
--
-- 실행하지 마세요. sql/03-auth.sql → sql/04-rls.sql 을 쓰세요.
-- 이미 실행했다면 04-rls.sql 이 이 정책을 지웁니다.
--
-- 아래는 이력 참고용입니다.
-- ──────────────────────────────────────────────────────────────
