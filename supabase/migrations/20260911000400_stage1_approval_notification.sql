...
create or replace function core.claim_due_notifications(
...
begin
  if p_limit < 1 or p_limit > 200 then
    raise exception '한 번에 처리할 알림 수는 1~200건이어야 합니다.' using errcode = '22023';
  end if;
  ...
  with due as (
    ...
    returning o.notification_id, o.template_code, o.recipient_user_id, o.channel, o.payload
  )
  select c.notification_id, c.template_code, c.recipient_user_id, u.email,
         c.channel, c.payload
    from claimed c
...
create or replace function core.finish_notification(
...
revoke all on function core.finish_notification(uuid, boolean, text, text) from public, anon, authenticated;
...
grant execute on function core.finish_notification(uuid, boolean, text, text) to service_role;
