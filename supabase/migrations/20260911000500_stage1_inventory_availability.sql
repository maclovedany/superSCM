...
-- 완료된 입고를 반영한 append-only 원장. 같은 입고(source_record_id)는 한 번만 반영된다
-- (유니크 제약 + on conflict do nothing) — 배치를 다시 커밋하거나 재반영을 여러 번 호출해도
-- 이중 계산되지 않는다. 행을 지우거나 고치지 않는다 — 반영 시점의 사실을 그대로 남긴다.
...
create unique index if not exists stock_receipt_ledger_source_uq
  on core.stock_receipt_ledger (source_record_id);
...
comment on table core.stock_receipt_ledger is
  '완료된 입고가 정상 창고재고를 늘린 append-only 이력. source_record_id당 한 행만 존재한다.
...
comment on function core.apply_stock_receipts_from_batch(uuid) is
  '권한 검사가 없는 내부 계산. core.commit_import_batch(goods_receipt 분기)만 부른다.
   같은 source_record_id는 두 번 반영되지 않는다(유니크 제약). 원장에는 조건 없이
   기록하고, core.stock_balance 반영 여부는 core.recompute_stock_balance_totals가 정한다';
...
--      (core.stock_receipt_ledger가 source_record_id당 한 번만 반영한다)
