...
--   null). 이 프로젝트는 아직 실제 창고 표기를 모르므로(메모리: 실데이터에 재고 없음) 창고전용
--   규칙은 일반적인 예시 코드(SERVICE_CENTER · PARTNER)만 시드한다 — 실제 표기가 확인되면
--   이 표에 행을 더하면 된다.
...
insert into core.inventory_scope_rule (warehouse_code, raw_status, scope_code, description) values
  ...
  -- 창고전용 규칙 (재고상태와 무관) — 실제 창고 표기가 확인되면 여기에 추가한다
  ('SERVICE_CENTER', null, 'SERVICE_CENTER', '서비스센터 창고는 재고상태와 무관하게 서비스센터 보유재고'),
  ('PARTNER',         null, 'PARTNER',        '파트너 창고는 재고상태와 무관하게 파트너 보유재고')
...
--      core.apply_stock_balance_from_batch를 호출해 정상 창고재고를 자동으로 반영한다.
--      core.commit_import_batch는 이미 함수 맨 위에서 core.is_admin()을 요구하므로 별도
--      권한 검사를 추가하지 않는다.
-- goods_receipt는 지금 core.stock_balance를 직접 늘리지 않는다 — core.v_open_po_qty
-- 참고 열만 줄인다. "재고 스냅샷 적재"와 "입고 건별 증가"를 같은 배치에서 동시에 반영하면
-- 이중 계산 위험이 있어, 지금은 스냅샷(raw.inventory) 쪽만 stock_balance의 근거로 삼는다.
...
begin
  ...
  if b.import_type = 'inventory' then perform core.apply_stock_balance_from_batch(p_batch_id); end if;
end; $$;

comment on function core.commit_import_batch(uuid) is
  'STEP 4 원본 + Task 4 확장: inventory·goods_receipt payload에 재고 분류 컬럼을 포함하고,
   inventory 배치 적재 직후 같은 트랜잭션에서 core.apply_stock_balance_from_batch를 호출한다';
...
-- (a-2) 창고 규칙이 상태를 이기는지 확인: 같은 '정상' 상태라도 warehouse_code가
-- 'SERVICE_CENTER'면 core.classify_inventory_scope('SERVICE_CENTER','정상')가
-- 'SERVICE_CENTER'를 돌려줘야 한다(NORMAL이 아니다).
