...
begin
  ...
  loop
    ...
    -- 재실행해도 실제 입고일은 보존한다 — confirmed_receipt_date만 최신화하고 actual_receipt_date · 이력은 건드리지 않는다.
...
comment on function core.build_procurement_schedule(uuid) is
  'Task 10b — PLAN_CONFIRM. 승인된(APPROVED) 계획만 허용한다. 1개월차 · 최종 발주량>0 라인마다 '
  '공급처 → 출항일 → 준비기간 → KR 영업일 확정 순서로 일정을 만든다. 재실행해도 행이 늘지 않고 '
  '이미 입력된 실제 입고일을 보존한다(plan_line_id unique + on conflict)';
...
comment on view analytics.v_procurement_schedule is
  ...
  'SCHEDULED인데 실제 입고일이 아직 없을 때의 ACTUAL_RECEIPT_UNSET이다';

-- 세 집계는 모두 core.receipt_schedule_result에서 같은 원천 행(SCHEDULED만)을 쓴다(판정 7) — 법인 ·
-- 품목 · 월별로 나눠도 n_total의 합은 항상 같다.
...
left join core.supply_entity ent on ent.entity_id = s.entity_id
where s.calculation_status = 'SCHEDULED'
