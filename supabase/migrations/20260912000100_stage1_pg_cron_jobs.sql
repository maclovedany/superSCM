...
-- );
--
-- 두 시크릿이 아직 없으면 아래 스케줄 등록 시 net.http_post의 url이 null이 되어 pg_net이
-- 요청을 보내지 못합니다(요청 자체는 net._http_response에 기록되지 않고 SQL 단계에서 조용히
-- null을 넘깁니다) — §5 확인 쿼리로 vault 시크릿 존재 여부를 먼저 확인하세요.
...
-- 최근 실행 이력 — status가 'failed'면 return_message에 원인이 남습니다(예: SQL 문법 오류).
-- pg_net 호출 자체(HTTP 성공/실패)는 이 표에 나타나지 않습니다 — job이 net.http_post를
-- "제출"했는지만 보여줍니다. 아래 net._http_response로 실제 HTTP 결과를 확인하세요.
...
limit 20;
-- 기대: status_code = 200(claimed 0건이어도 정상), 401이면 vault의 stage1_notify_secret이
-- Edge Function의 CRON_SECRET과 다릅니다. status_code가 비어 있고 error_msg만 있으면 아직
-- 응답을 못 받았거나(요청 실패) url 자체가 null입니다(§2 vault 시크릿 미설정).
