...
--   hoc_item이 다시 다른 행의 related_item으로 이어지는 다단계 체인은 남아 있고, 그 경우
--   품목이 최종 대표코드가 아니라 중간 코드로 귀속된다(총량은 보존되므로 순수 귀속 오류다).
--   체인에는 순환이 있어 "끝까지 따라가기"가 종료하지 않으므로, 해소하려면 정본 대표코드를
--   정하는 명시적 규칙이 따로 필요하다 — 별도 과제(수치는 아직 재측정 중이라 여기 적지
--   않는다 — chain-brief.md 참고, 확정되면 그쪽에 기록한다).
...
comment on view core.v_part_linkage is
  ...
  단계)만 고쳤다 — hoc_item이 다시 다른 행의 related_item으로 이어지는 다단계 체인은 남아
  있고, 그 경우 품목이 최종 대표코드가 아니라 중간 코드로 귀속된다(총량은 보존되므로 순수
  귀속 오류). 체인에는 순환이 있어 끝까지 따라가기가 종료하지 않으므로, 해소하려면 정본
  대표코드를 정하는 명시적 규칙이 따로 필요하다 — 별도 과제';
