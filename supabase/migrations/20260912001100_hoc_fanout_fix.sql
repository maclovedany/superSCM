...
--
-- ★★ 이 수정은 1홉(raw.bridge_xcn 한 테이블, related_item → hoc_item 한 단계) 기준이다.
--   hoc_item이 다시 다른 행의 related_item으로 이어지는 다단계 체인은 고치지 않는다(리뷰
--   확정 실측 — 체인 간선 547개 · 비종단으로 귀속되는 매핑 460건 · 최대 깊이 15, 출고 영향
--   553행·수량 1,636·품목 114개 = 전체의 0.0347%). 총량은 어느 쪽이든 보존되므로 순수 귀속
--   오류다 — 이 114개 품목은 04-core-views.sql:89가 약속하는 "이 뷰를 쓰면 살아 있는 부품이
--   단종으로 안 보인다"가 아직 완전히 성립하지 않는 상태로 남아 있다(팬아웃 결함과는 다른
--   결함, 이번 수정 범위 밖 — 별도 라운드). 종단(다단계) 추적은 그 자체로 종료하지 않는다 —
--   raw.bridge_xcn에 상호 순환쌍이 17개 있다(예: 724K86001 ↔ 566K26335R78). 재귀 CTE로
--   시도하더라도 순환 방지 로직 없이는 무한 루프이고, 종단 기준의 "진짜 모호"는 이 1홉
--   기준의 2건과 집합 자체가 다르다(리뷰 실측 3건) — 다음에 이 뷰를 다시 열 때 참고.
...
comment on view core.v_part_linkage is
  ...
  냈다(실측, 스크래치 DB) — 이 수정으로 셋이 모두 일치한다. ★ 1홉 기준(raw.bridge_xcn 한
  단계)만 고쳤다 — hoc_item이 다시 related_item으로 이어지는 다단계 체인(리뷰 실측: 간선 547·
  비종단 귀속 460건·출고 영향 0.0347%)은 별도 라운드로 남아 있다(raw.bridge_xcn에 상호 순환쌍
  17개가 있어 종단 추적은 재귀 CTE만으로 끝나지 않는다)';
