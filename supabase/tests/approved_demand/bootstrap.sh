#!/bin/bash
# 로컬 PostgreSQL에 검증 DB를 만든다 — error.md #14 · #15 · #21 절차, supabase/tests/demand_submission과 동일 구조
#   클러스터 역할 확인 → createdb → auth 스텁 → schema-dump → STEP 4 · 7 정책 선삭제 → 전체 마이그레이션
#   → 0800(Task 8) 한 번 더 적용(재실행 안전성)
...
TARGET_MIGRATION="$REPO/supabase/migrations/20260911000800_stage1_approved_demand.sql"
