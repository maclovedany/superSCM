#!/bin/bash
# 확정 수요 구성(Task 8) DB 검증 전체 실행 — 로컬 PostgreSQL 임시 DB 전용
#   임시 DB 생성(bootstrap) → fixture → 시나리오 → 요약 → 임시 DB 삭제
# 사용: bash supabase/tests/approved_demand/run-all.sh [scm_test_* DB 이름]
...
DB=${1:-scm_test_approved_$(date +%Y%m%d%H%M%S)}
require_local_target "$DB"
LOG_DIR=${LOG_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/scm_test_approved.XXXXXX")}
mkdir -p "$LOG_DIR"
