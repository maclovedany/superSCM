#!/bin/bash
# 부서별 월간 수요 제출 · 마감 관리 DB 검증 전체 실행 — 로컬 PostgreSQL 임시 DB 전용
#   임시 DB 생성(bootstrap) → fixture → 시나리오 → 동시성(fix round 3) → 요약 → 임시 DB 삭제
# 사용: bash supabase/tests/demand_submission/run-all.sh [scm_test_* DB 이름]
...
DB=${1:-scm_test_demand_$(date +%Y%m%d%H%M%S)}
require_local_target "$DB"
LOG_DIR=${LOG_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/scm_test_demand.XXXXXX")}
mkdir -p "$LOG_DIR"
