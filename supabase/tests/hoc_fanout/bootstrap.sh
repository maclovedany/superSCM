#!/bin/bash
# 로컬 PostgreSQL에 검증 DB를 만든다 — error.md #14 · #15 · #21 절차, supabase/tests/chart_views와 동일 구조
#   클러스터 역할 확인 → createdb → auth 스텁 → schema-dump → STEP 4 · 7 정책 선삭제 → 전체 마이그레이션
#   → 20260912001100(core.v_part_linkage 팬아웃 수정) 한 번 더 적용(재실행 안전성)
...
TARGET_MIGRATION="$REPO/supabase/migrations/20260912001100_hoc_fanout_fix.sql"
...
# 전체 마이그레이션 적용. 20260912001100(HOC 팬아웃 수정)은 자기 순서 자리에서 곧바로 한 번 더
# 적용한다(재실행 안전성 — core.v_part_linkage 열 구성은 그대로라 create or replace가 충돌 없이
# 재적용돼야 한다).
