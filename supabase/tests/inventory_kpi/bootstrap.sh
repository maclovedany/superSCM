#!/bin/bash
# 로컬 PostgreSQL에 검증 DB를 만든다 — error.md #14 · #15 · #21 절차, supabase/tests/item_policy와 동일 구조
#   클러스터 역할 확인 → createdb → auth 스텁 → schema-dump → STEP 4 · 7 정책 선삭제 → 전체 마이그레이션
#   → 001150(Task 12) 한 번 더 적용(재실행 안전성)
...
TARGET_MIGRATION="$REPO/supabase/migrations/20260911001150_stage1_inventory_kpi.sql"
...
# 001150(Task 12)은 자기 순서 자리에서 곧바로 한 번 더 적용한다(재실행 안전성). 이 마이그레이션은
# 뒤에 아무 마이그레이션도 확장하지 않으므로 error.md #24(뒤 마이그레이션이 덧붙인 열을 앞 정의가
# 지우는 문제)는 여기서는 발생하지 않는다 — 그래도 관례를 맞춰 재실행 안전성을 그 자리에서 확인한다.
