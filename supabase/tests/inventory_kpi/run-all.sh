#!/bin/bash
# 월말 재고 성과·운영 기준월(Task 12) DB 검증 전체 실행 — 로컬 PostgreSQL 임시 DB 전용
#   임시 DB 생성(bootstrap) → fixture → 시나리오 → 요약 → 임시 DB 삭제
# 사용: bash supabase/tests/inventory_kpi/run-all.sh [scm_test_* DB 이름]
...
DB=${1:-scm_test_invkpi_$(date +%Y%m%d%H%M%S)}
require_local_target "$DB"
LOG_DIR=${LOG_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/scm_test_invkpi.XXXXXX")}
mkdir -p "$LOG_DIR"
...
for scenario in "S1 " "S1b" "S2 " "S3 " "S4 " "S5 " "S6 " "S7 " "S8 " "S8b" "S8c" "S9 " "S10"; do
