#!/bin/bash
# 로컬 PostgreSQL에 검증 DB를 만든다 — supabase/tests/item_policy와 동일 구조(error.md #14 · #15 · #21).
#   클러스터 역할 확인 → createdb → auth 스텁 → schema-dump → STEP 4 · 7 정책 선삭제 → 전체 마이그레이션
#   → 0950(Task 10a) 한 번 더 적용(재실행 안전성 — error.md #24처럼 뒤 마이그레이션이 이 뷰를
...
TARGET_MIGRATION="$REPO/supabase/migrations/20260911000950_stage1_master_edit.sql"
...
# 0950은 자기 순서 자리에서 곧바로 한 번 더 적용한다(재실행 안전성. error.md #24와 같은 이유 —
# 이후 마이그레이션이 이 파일의 뷰를 넓히면, 전체 적용 뒤 0950만 다시 돌리는 것은 실패할 수 있다).
