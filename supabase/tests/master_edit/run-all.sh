#!/bin/bash
# Task 10a 관리자 마스터 편집(해외법인·공급처·출항일 규칙·영업일 달력) DB 검증 전체 실행 — 로컬 PostgreSQL 임시 DB 전용
#   임시 DB 생성(bootstrap) → fixture → 시나리오 → 요약 → 임시 DB 삭제
# 사용: bash supabase/tests/master_edit/run-all.sh [scm_test_* DB 이름]
...
DB=${1:-scm_test_masteredit_$(date +%Y%m%d%H%M%S)}
require_local_target "$DB"
LOG_DIR=${LOG_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/scm_test_masteredit.XXXXXX")}
mkdir -p "$LOG_DIR"
...
for scenario in S1 S2 S3 S4 S5 S6 S7 S8 S9 S10 S11 S12 S13 S14 S15 S16 S17 S18; do
