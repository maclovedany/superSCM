#!/bin/bash
# 로컬 PostgreSQL에 검증 DB를 만든다 — error.md #14 · #15 · #21 · #31 절차, supabase/tests/item_policy와 동일 구조
#   클러스터 역할 확인 → createdb → auth 스텁 → schema-dump → STEP 4 · 7 정책 선삭제 → 전체 마이그레이션
#   → 20260912000400(Task 15) 한 번 더 적용(재실행 안전성)
...
TARGET_MIGRATION="$REPO/supabase/migrations/20260912000400_stage1_practice_dataset.sql"
...
for migration in "$REPO"/supabase/migrations/*.sql; do
  ...
  if ! "${PSQL[@]}" -d "$DB" -f "$migration" > "$LOG_DIR/migration-$name.log" 2>&1; then
    # error.md #31 — pg_cron · pg_net은 Supabase 플랫폼 전용 확장이라 일반 로컬 PostgreSQL에 없다.
    # 그 이유로 실패한 파일만 건너뛴다. 다른 이유면 지금까지와 같이 즉시 멈춘다. 이 스위트는
    # pg_cron 작업을 검증하지 않으므로 건너뛰어도 이후 마이그레이션의 스키마 상태에 영향이 없다.
    if grep -qE 'extension "(pg_cron|pg_net)" is not available' "$LOG_DIR/migration-$name.log"; then
      echo "건너뜀(로컬에 확장 없음, error.md #31): $name" >&2
      continue
    fi
  ...
  # Task 15 파일은 자기 순서 자리에서 곧바로 한 번 더 적용한다(재실행 안전성. error.md #24와 같은 이유).
