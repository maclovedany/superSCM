...
#   → 20260911001000(Task 10b) 한 번 더 적용(재실행 안전성 — 이후 마이그레이션이 이 파일의 뷰를 넓히게
#   되면 그 시점에도 재실행이 안전한지 이 자리에서 확인한다)
...
TARGET_MIGRATION="$REPO/supabase/migrations/20260911001000_stage1_procurement_schedule.sql"
...
# 0900(procurement_plan)은 item_policy 뷰를 넓힌다 — item_policy 스위트의 bootstrap과 같은 이유로,
# 전체 적용 뒤 옛 좁은 정의로 되돌아가는 재실행은 하지 않는다(error.md #24). 여기서는 우리 대상
# 마이그레이션(20260911001000)만 자기 순서 자리에서 재실행 안전성을 확인한다.
