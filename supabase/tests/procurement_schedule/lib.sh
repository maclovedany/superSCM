# supabase/tests/procurement_schedule 공통 설정 — 로컬 PostgreSQL 임시 DB에서만 실행한다.
# (master_edit · procurement_plan과 같은 구조 — 이 파일은 그대로 복제했다)
#
...
# 세션 timezone GUC에 따라 date↔timestamptz 변환 결과가 달라질 수 있다(error.md Task 7 fix round 1).
# 이 스위트는 항상 명시적인 세션 timezone에서 돈다 — 기본은 UTC. 다른 timezone에서 다시 실행하려면
# PGOPTIONS="-c timezone=America/Los_Angeles" bash run-all.sh처럼 미리 지정한다.
