#!/bin/bash
# Task 10b 발주 일정 · 입고 차이 DB 검증 전체 실행 — 로컬 PostgreSQL 임시 DB 전용
#   임시 DB 생성(bootstrap) → fixture → 시나리오 → 요약 → 임시 DB 삭제
# 사용: bash supabase/tests/procurement_schedule/run-all.sh [scm_test_* DB 이름]
...
DB=${1:-scm_test_procsched_$(date +%Y%m%d%H%M%S)}
require_local_target "$DB"
LOG_DIR=${LOG_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/scm_test_procsched.XXXXXX")}
mkdir -p "$LOG_DIR"
...
if ! "${PSQL[@]}" -f "$HERE/fixtures.psql" > "$LOG_DIR/fixtures.log" 2>&1; then
  echo "결과: fixture 실패"; tail -20 "$LOG_DIR/fixtures.log"
  exit 1
fi
...
for scenario in S1 S2 S3 S4 S5 S6 S7 S8 S9 S10 S11 S12 S13 S14 S14b S15 S16 S17 S18 S19 S20 S21 S22 S23 \
                S24 S25 S26 S27 S28 S29; do
