...
| `scenarios.psql` | S2 PARTIAL/WAIT_FULL · S3 만료 불변 · S4 확정/확정배정 취소/재등록 · S5 수동배정/승인 · S6 우선순위 · S7 권한/직접 쓰기 차단 · S8 주문 취소 · S9 만료 시각 이후 차단 · S10 만료 뒤 FIRM · 확보만 남은 주문의 수주 확정 · **S11(Task 6) 30일 자동 만료 경계 · 재실행 · FIRM 유지 · 배정 0건 WAITING_FULL 만료** · **S12(Task 6) 입고 후 AUTO 자동 배정 대기 순번 · CONFIRMED FIRM · WAIT_FULL 스킵 · 만료 주문 건너뜀** · **S13(Task 6) MANUAL 품목 자동 배정 0건 · 처리 필요 알림 · 대기 순번 조회** · S14 다품목 문서번호 upsert 적재 · **S15(보정 2026-09-12, 라운드 1) Open PO·이동 중(참고) 두 참고 열 모두 콤마·공란·정크 텍스트와 출처 게이트를 검증한다 — core.v_open_po_qty · analytics.v_available_stock · core.v_inbound_qty 조회가 예외 없이 끝나고(select count(*) from (select * from 뷰) t 로 컬럼 프루닝 없이 확인), 파싱 불가 값이 부분합이 아니라 null로 남으며, 출처(batch_id) 없는 행은 파싱 가능 여부와 무관하게 절대 숫자로 보이지 않는다(출처 게이트, 두 열 모두) · analytics.v_stock_reference_source_status(화면 배너용 상태 뷰, 열 추가 대신 별도 객체)가 두 열의 사유를 함께 알려준다 · 적재 경로(core.apply_stock_receipts_from_batch)는 콤마는 받아들이되 파싱 불가면 배치 전체를 명확한 오류로 거부한다(조용히 행만 빼지 않는다)** · **S16(Task 17, 2026-09-12) core.v_item_master 출처 게이트(raw.item_master.batch_id is not null) — 출처 없는 품목은 전체 열 조회(select * 로 컬럼 프루닝 회피)에서 걸러지고, 그 품목으로는 core.create_sales_order가 ORDER_ITEM_UNKNOWN으로 거절하며(회귀가 아니라 강화), 출처 있는 품목·stock_balance 배경 기존 품목은 영향 없다 · core.v_item_master_source_status·analytics.v_item_master_source_status(새 상태 객체, v_stock_reference_source_status와 별개)가 걸러진 행 수와 사유코드(ITEM_MASTER_SOURCE_UNVERIFIED)를 보고한다** |
