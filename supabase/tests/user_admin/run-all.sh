#!/bin/bash
# 관리자 계정 관리(core.admin_upsert_app_user_profile 등) DB 검증 전체 실행 — 로컬 PostgreSQL 임시 DB 전용
#   임시 DB 생성(bootstrap) → fixture → 시나리오 → 요약 → 임시 DB 삭제
# 사용: bash supabase/tests/user_admin/run-all.sh [scm_test_* DB 이름]
...
DB=${1:-scm_test_useradmin_$(date +%Y%m%d%H%M%S)}
require_local_target "$DB"
LOG_DIR=${LOG_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/scm_test_useradmin.XXXXXX")}
mkdir -p "$LOG_DIR"
...
for scenario in S1 S2 S3 S4 S5 S6 S7 S8 S9 S10 S11 S12 S13 S14 S15 S16; do
